...
class Orchestrator {
    ...
    _initialMessages({ cfg, view, selection, userText, attachments }) {
        const parts = [
            ...
            'Available tool groups: view_* (gauge editing, layout, animations), device_* (device CRUD, connection config), tag_* (tag browsing, values, creation), project-level: view_list/create/delete, script_list/run, alarm_list.',
            ...
            'To add animations, use view_add_gauge or view_update_gauge with these parameters:',
            '- ranges: [{min, max, color, stroke?, text?}] — conditional coloring based on tag value. E.g. ranges: [{min:0,max:30,color:"#00ff00"},{min:30,max:70,color:"#ffaa00"},{min:70,max:100,color:"#ff0000"}]',
            '- actions: [{variableId, type, range?, options?}] — tag-triggered animations.',
            '  Action types: blink (toggle fillA/fillB/strokeA/strokeB at interval ms),',
            '  rotate (spin element, options: {minAngle, maxAngle, delay}),',
            '  clockwise/anticlockwise (continuous rotation),',
            '  move (reposition, options: {toX, toY, duration}),',
            '  color (change fill/stroke when condition met),',
            '  hide/show (toggle visibility), stop/downup.',
            '  Blink example: actions: [{variableId:"tag_123",type:"shapes.action-blink",range:{min:1,max:1},options:{fillA:"#00ff00",fillB:"#ff0000",interval:500}}]',
            '  Rotate example: actions: [{variableId:"tag_123",type:"shapes.action-rotate",range:{min:0,max:100},options:{minAngle:0,maxAngle:360,delay:0}}]',
            '- tagId: bind a tag to the gauge value slot. Also supports view_bind_tag for multiple slots (value, visibility, blink, color, rotate).',
            '- events: [{type, action, actparam, actoptions}] — click/dblclick handlers. type=shapes.event-click, action=shapes.event-onsetvalue to write a tag value on click.',
            '',
            '=== DATA SOURCE CONFIGURATION ===',
            'To connect a data source: 1) device_add to create device, 2) device_update_property to set connection params, 3) tag_add to create tags, 4) view_update_gauge to bind tags.',
            'Protocol connection examples:',
            '  OPC-UA: device_update_property with {endpoint:"opc.tcp://192.168.1.100:4840"}',
            '  Modbus TCP: device_update_property with {address:"192.168.1.100",port:502,unitId:1}',
            '  MQTT: device_update_property with {address:"mqtt://broker:1883",clientId:"fuxa",topic:"sensors/#"}',
            '  Siemens S7: device_update_property with {address:"192.168.1.100",rack:0,slot:1}',
            'Tag address formats: OPC-UA "ns=2;s=Temperature", Modbus "40001" (holding register), MQTT "sensors/temp".'
        ];
